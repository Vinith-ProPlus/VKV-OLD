...
class CityController extends Controller{
    ...
    public function save(Request $req){
		if($this->general->isCrudAllow($this->CRUD,"add")==true){
			...
			$rules=array(
				...
				'CityName' =>['required','min:3','max:100',new ValidUnique(array("TABLE"=>$this->generalDB."tbl_cities","WHERE"=>" CityName='".$req->CityName."' and CountryID='".$req->CountryID."' and StateID='".$req->StateID."' and DistrictID='".$req->DistrictID."' "),"This City Name is already taken.")],
			);
			$message=array();
    ...
    public function update(Request $req,$CityID){
		if($this->general->isCrudAllow($this->CRUD,"edit")==true){
			...
			$rules=array(
				...
				'DistrictID' =>['required',$ValidDB['District']],
				'PostalCodeID' =>['required',$ValidDB['PostalCode']],
				'CityName' =>['required','min:3','max:100',new ValidUnique(array("TABLE"=>$this->generalDB."tbl_cities","WHERE"=>" CityName='".$req->CityName."' and CountryID='".$req->CountryID."' and StateID='".$req->StateID."' and DistrictID='".$req->DistrictID."' and CityID<>'".$CityID."' "),"This City Name is already taken.")],
			);
			$message=array();
			$validator = Validator::make($req->all(), $rules,$message);
	...
	public function TableView(Request $req){ 
		if($this->general->isCrudAllow($this->CRUD,"view")==true){
			...
			$data=array();
			$data['POSTDATA']=$req;
			$data['TABLE']=$this->generalDB.'tbl_cities as CI LEFT JOIN '.$this->generalDB.'tbl_postalcodes as P ON P.PID = CI.PostalID LEFT JOIN '.$this->generalDB.'tbl_districts as D ON D.DistrictID = CI.DistrictID LEFT JOIN '.$this->generalDB.'tbl_states as S ON S.StateID = CI.StateID LEFT JOIN '.$this->generalDB.'tbl_countries as C ON C.CountryID = CI.CountryID';
	...
	public function TrashTableView(Request $request){
		if($this->general->isCrudAllow($this->CRUD,"restore")==true){
			...
			$data['TABLE']=$this->generalDB.'tbl_cities as C';
			$data['PRIMARYKEY']='CityID';
			$data['COLUMNS']=$columns;
			$data['COLUMNS1']=$columns1;
			$data['GROUPBY']=null;
