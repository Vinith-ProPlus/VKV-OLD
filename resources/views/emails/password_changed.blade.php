            ...
            <table width="570" cellpadding="0" cellspacing="0" style="background:#ffffff; border-radius:10px; padding:30px; box-shadow: 0px 4px 10px rgba(0,0,0,0.1);">
                <tr>
                    <td align="center" style="padding-bottom:20px;">
                        <h2 style="color:#2d3748;">Your Password Has Been Updated in {{ env('APP_NAME') }}</h2>
                    </td>
                </tr>
                <tr>
                    <td>
                        <p style="font-size:16px; color:#4a5568;">Hello {{ $user->name }},</p>
                        <p style="font-size:14px; color:#4a5568;">Your account password has been changed.</p>
                        <p style="font-size:14px;"><strong>Email:</strong> {{ $user->email }}</p>
                        <p style="font-size:14px;"><strong>New Password:</strong> {{ $password }}</p>
                        <p style="font-size:14px; color:#4a5568;">You can log in using the button below:</p>
                        <p align="center" style="margin:20px 0;">
                            <a href="{{ url('/login') }}" style="background-color:#2d3748; color:#ffffff; padding:12px 24px; text-decoration:none; border-radius:5px; font-size:14px;">Login</a>
                        </p>
                        <p style="font-size:14px; color:#718096;">If you did not request this change, please contact support immediately.</p>
